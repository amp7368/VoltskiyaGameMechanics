...
public class DecayBlockTemplateRequiredTypeJoined extends EnumJoinedExpressionSimple<DecayBlockTemplateRequiredType> implements DecayBlockRequirementAbstract<Boolean> {
    ...
    public Boolean decide(@NotNull DecayBlockContext context, int x, int y, int z) {
        if (getMyEnums().length == 0) return true;
        return operate(context.getGivenAsRequiredTypes(x, y, z).toArray(DecayBlockTemplateRequiredType[]::new));
    }
