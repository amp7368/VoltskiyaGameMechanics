...
import static voltskiya.apple.game_mechanics.tmw.sql.SqlVariableNames.MOB_MY_UID;
import static voltskiya.apple.game_mechanics.tmw.sql.SqlVariableNames.MOB_UNIQUE_NAME;
import static voltskiya.apple.game_mechanics.tmw.sql.SqlVariableNames.TABLE_STORED_MOB;
import static voltskiya.apple.game_mechanics.tmw.sql.SqlVariableNames.WORLD_MY_UID;
import static voltskiya.apple.game_mechanics.tmw.sql.SqlVariableNames.X;
import static voltskiya.apple.game_mechanics.tmw.sql.SqlVariableNames.Z;
...
public class MobSqlStorage extends AppleRequestLazyService<Boolean> {
    ...
    public static void getMobs(int lowerX, int upperX, int lowerZ, int upperZ, int worldMyUid,
        Consumer<List<TmwStoredMob>> callback) {
        instance.queue("get_mobs", () -> {
            List<TmwStoredMob> finalAnswer = getMobThreaded(lowerX, upperX, lowerZ, upperZ,
                worldMyUid);
            Bukkit.getScheduler()
                .scheduleSyncDelayedTask(VoltskiyaPlugin.get(), () -> callback.accept(finalAnswer));
            return true;
        }, AppleRequestOnConflict.ADD());
    }
    ...
    public static void removeMobsThreaded(Collection<Long> mobsToDelete) {
        ...
        Session session = VerifyDatabaseTmw.sessionFactory.openSession();
        CriteriaBuilder criteriaBuilder = session.getCriteriaBuilder();
        CriteriaDelete<TmwStoredMob> query = criteriaBuilder.createCriteriaDelete(
            TmwStoredMob.class);
        Root<TmwStoredMob> mobRoot = query.from(TmwStoredMob.class);
        Transaction transaction = session.beginTransaction();
    ...
    public static Map<Long, SpawnPercentages> getRegen() {
        ...
        @SuppressWarnings("unchecked") NativeQuery<TmwSpawnPercentagesResponse> nativeQuery = session.createNativeQuery(
            queryString, "tmw_spawn_percentages");
        ...
        for (TmwSpawnPercentagesResponse s : results) {
            resultsInMap.compute(s.chunkUid, (k, spawnPercentages) -> {
                if (spawnPercentages == null) {
                    if (s.biome == null) {
                        return null; //shouldn't happen in production
                    }
                    return new SpawnPercentages(s.biome, s.chunkX, s.chunkZ, s.mobName,
                        s.worldMyUid, s.middleX, s.middleY, s.middleZ, s.mobCount);
                } else {
