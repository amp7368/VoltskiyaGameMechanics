...
public class GetChunksRequest {
    ...
    private final Map<Pair<Integer, Integer>, ChunkSnapshot> chunks;
    private final Consumer<Map<Pair<Integer, Integer>, ChunkSnapshot>> onDone;

    public GetChunksRequest(Map<Pair<Integer, Integer>, ChunkSnapshot> chunksToLoad,
        Location locationToGetBiome, Consumer<Map<Pair<Integer, Integer>, ChunkSnapshot>> onDone) {
        this.chunks = chunksToLoad;
        this.onDone = onDone;
        int count = 0;
        for (ChunkSnapshot addMe : chunks.values()) {
            if (addMe == null) {
                count++;
            }
        }
